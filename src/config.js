...
/* =========================================================================
   功能开关 FLAGS（每个新系统一个，关掉后游戏必须还能正常跑）
   -------------------------------------------------------------------------
   铁律：任何一个 FLAG 置 false，游戏不得报错、不得缺功能崩溃。
   这是「出问题能单独回滚」的唯一保障（v4 §12.2 / v5 A3）。
   ========================================================================= */
const FLAGS = {
...
/* =========================================================================
   ...
   ① 对称：玩家与敌方英雄走同一套公式（同一张 HEROES 表、同一套士气规则）。
      玩家的强度只能来自「局内花金币升级」，不是「玩家有英雄」。
      对称的是能力框架，不是获取路径 —— 这是 P4「对称即公平」的落地。

   ② 与指令系统分轨：英雄大招走独立的「士气」资源，不消耗 CP。
      若共享 CP，玩家最优解永远是攒给大招，6 条指令全部变摆设
      ——一个系统吃掉另一个系统是设计事故。

   ③ 英雄不死亡，只退场（retreatTime 后以 50% HP 返场）：
      会死 → 玩家永远不敢出击，与守城「驻守/出击」的张力直接冲突，
      且一次失误就失去英雄会产生挫败感。退场是「有时间代价但不毁灭」的惩罚。
   ========================================================================= */
const HERO_CFG = {
  moraleMax: 100,
  moraleRegen: 1.6,     // 每秒自然回复。100/1.6 ≈ 62s 纯自然满；实战有击杀与挨打，约 45-55s 一发
  moralePerKill: 3,     // 己方任意单位击杀 → 该侧英雄 +3（不是只有英雄自己击杀，否则英雄抢人头成最优解）
  moralePerHit: 2,      // 英雄自身每次受击 +2（挨打攒怒气）。按「次」不按「量」，避免被高伤单位刷爆
  retreatTime: 30,      // 退场时长（秒）。v5 定 30s；一轮内可观察，若节奏拖沓改 20s
  retreatHpPct: 0.5,    // 返场 HP 百分比
  hpMul: 4,             // 相对同等级近战。能扛 3-4 个单位集火约 8 秒，够玩家反应
  atkMul: 2.2,          // 输出约等于 2 个兵。不是「一打十」，避免英雄抢戏
  speed: 200,           // 介于近战 160 与骑兵 300 之间：能跟上阵型，追不上骑兵
  radius: 22,           // 比普通单位（13-17）大一圈，视觉上"这是个大人物"
  auraHardCap: 0.6,     // 光环减伤硬上限：防止光环 + 大招 + 其他增益叠加破表
};

const HEROES = {
  guardian: {
    key: "guardian", name: "铁壁将军", short: "盾",
    atkType: "blunt", armorType: "hero",
    color: "#c9b037",
    desc: "防守核心：范围友军减伤，大招免控加固",
    // 光环：常驻、不占操作带宽。12% 减伤 ≈ 有效 HP +13.6%，
    // 略低于战术卡「铁血纪律」的 +15% HP —— 因为光环是范围性的，覆盖面更广，单体价值要打折。
    aura: { radius: 180, reduce: 0.12 },
    ult: {
      name: "不动如山", cd: 50, dur: 5,
      desc: "5 秒内：范围内友军免疫击退、受伤再减 30%；守城模式额外修复城墙 15%",
      immuneKnock: true,     // 免击退：在 knockUnit 里拦截
      extraReduce: 0.30,     // 叠加在光环上，合计 12% + 30% = 42%（低于 auraHardCap 0.6）
      wallHealPct: 0.15,     // 守城模式专用
    },
  },
};
